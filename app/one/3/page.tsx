...
// greeting{ imco(i, dict) }artist[i].map(art => () )[0][0]lii(artists, i)
  rows = (i=0) => <Row id='scrw' className={ 'r'+i+' ac' } style={{ width: nextId+'vw' }}>{ imco(i) }
    ...
    <Button id="addc" onClick={ () => { let arts = artists; nextId+=15
      ...
      tr?.classList.add('df'); let tem = document.querySelector(".fon.p-2")
      tem?.classList.add('modal-cont')
      setTimeout(() => { tr?.classList.remove('df'); tem?.classList.remove('modal-cont') }, 1000)
      // console.log(artists)
